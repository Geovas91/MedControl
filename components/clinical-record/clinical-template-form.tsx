...
export function ClinicalTemplateForm({ action, initialValues, cancelHref }: { action: (state: State, formData: FormData) => Promise<State>; initialValues: TemplateFormValues; cancelHref: string }) {
  ...
  return <form action={formAction} className="glass-card-strong grid gap-5 p-4 sm:p-6">
    ...
    <label className="clinical-surface flex min-h-11 items-center gap-3 px-4 py-3 text-sm text-slate-700"><input name="is_active" type="checkbox" defaultChecked={values.isActive} className="h-4 w-4 rounded border-slate-300 text-clinic" />Disponible en formularios nuevos</label><input type="hidden" name="expected_updated_at" value={values.expectedUpdatedAt} />
