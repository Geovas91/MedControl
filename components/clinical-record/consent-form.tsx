...
export function PatientConsentForm({ action, cancelHref, templates }: { action: (state: FormState, formData: FormData) => Promise<FormState>; cancelHref: string; templates: Template[] }) {
  ...
  const [consentVersion, setConsentVersion] = useState(values.consentVersion);
  const [content, setContent] = useState(values.consentText);
  return <form action={formAction} className="glass-card-strong grid gap-5 p-4 sm:p-6">
    ...
    {templates.length ? <Field label="Plantilla de consentimiento" htmlFor="template_id"><Select id="template_id" name="template_id" value={templateId} onChange={(event) => { const nextTemplateId = event.target.value; setTemplateId(nextTemplateId); const template = templates.find((item) => item.id === nextTemplateId); if (template) { setConsentType(template.name); setConsentVersion("v1"); setContent(getTemplateContent(template.template_schema)); } }}><option value="">Sin plantilla</option>{templates.some((template) => template.is_system_template) ? <optgroup label="Recomendadas por CliniControl">{templates.filter((template) => template.is_system_template).map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</optgroup> : null}{templates.some((template) => !template.is_system_template) ? <optgroup label="Mi clínica">{templates.filter((template) => !template.is_system_template).map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</optgroup> : null}</Select></Field> : null}
    <p className="text-sm text-slate-600">Tipo, versión y texto se guardarán como el snapshot de este consentimiento. Puedes ajustarlos sin modificar la plantilla original.</p>
    <div className="grid gap-4 md:grid-cols-2"><Field label="Tipo de consentimiento *" htmlFor="consent_type"><Input id="consent_type" name="consent_type" value={consentType} onChange={(event) => setConsentType(event.target.value)} required maxLength={160} />{state.errors?.consentType ? <span className="text-xs text-rose-700">{state.errors.consentType}</span> : null}</Field><Field label="Versión *" htmlFor="consent_version"><Input id="consent_version" name="consent_version" value={consentVersion} onChange={(event) => setConsentVersion(event.target.value)} required maxLength={80} />{state.errors?.consentVersion ? <span className="text-xs text-rose-700">{state.errors.consentVersion}</span> : null}</Field></div>
