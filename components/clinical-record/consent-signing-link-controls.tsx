...
export function ConsentSigningLinkControls({ action, emailAction, revokeAction, cancelAction, hasActiveLink, expectedUpdatedAt, patientEmail, signingTokenExpiresAt, signingTokenUsedAt, signingTokenRevokedAt, signingActionsBlocked = false, signingActionsBlockedMessage, onActiveLinkChange }: { action: (state: SigningLinkState, formData: FormData) => Promise<SigningLinkState>; emailAction: (state: EmailState, formData: FormData) => Promise<EmailState>; revokeAction: () => Promise<void>; cancelAction: (formData: FormData) => Promise<void>; hasActiveLink: boolean; expectedUpdatedAt: string; patientEmail: string | null; signingTokenExpiresAt: string | null; signingTokenUsedAt: string | null; signingTokenRevokedAt: string | null; signingActionsBlocked?: boolean; signingActionsBlockedMessage?: string; onActiveLinkChange?: (active: boolean, updatedAt?: string) => void }) {
  ...
  return <section className="clinical-surface mt-6 p-4">
    ...
    {state.url ? <div className="glass-card mt-4 p-3"><p className="break-all text-sm text-slate-700">{state.url}</p><Button type="button" variant="ghost" className="mt-2" disabled={signingActionsBlocked} onClick={() => { void navigator.clipboard.writeText(state.url!); setCopied(true); }}><Copy className="h-4 w-4" />{copied ? "Copiado" : "Copiar enlace"}</Button></div> : null}
    ...
    <form action={cancelAction} className="glass-divider mt-6 border-t pt-5"><label htmlFor="cancellation_reason" className="text-sm font-semibold text-ink">Cancelar consentimiento</label><p className="mt-1 text-xs leading-5 text-slate-500">Solo es posible antes de la firma. El consentimiento se conservará como evidencia.</p><input id="cancellation_reason" name="cancellation_reason" maxLength={500} placeholder="Motivo opcional" className="glass-input mt-3 h-11 w-full rounded-xl px-3 text-sm text-ink outline-none focus:border-clinic focus:ring-4 focus:ring-teal-100/80" /><Button type="submit" variant="secondary" className="mt-3 border-rose-300 text-rose-700 hover:bg-rose-50"><XCircle className="h-4 w-4" />Cancelar consentimiento</Button></form>
    <dialog ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="consent-qr-title" aria-describedby="consent-qr-description" onClose={() => document.getElementById("consent-qr-trigger")?.focus()} className="glass-card-strong w-[calc(100%-2rem)] max-w-md p-0 shadow-dialog backdrop:bg-slate-950/50 backdrop:backdrop-blur-sm">
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><h2 id="consent-qr-title" className="text-lg font-bold text-ink">QR de firma</h2><p id="consent-qr-description" className="mt-2 text-sm leading-6 text-slate-600">Escanea este código con el celular del paciente para abrir el consentimiento.</p></div><button type="button" onClick={() => dialogRef.current?.close()} className="glass-control grid h-9 w-9 shrink-0 place-items-center text-slate-500 hover:text-ink" aria-label="Cerrar QR"><X className="h-4 w-4" /></button></div>
