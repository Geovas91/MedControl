"use client";

import { useActionState, useRef, useState } from "react";
import { useFormStatus } from "react-dom";
import { CheckCircle2, X } from "lucide-react";
import { Button } from "@/components/ui/button";

type FormState = { error?: string };

function FinalizeSubmitButton() {
  const { pending } = useFormStatus();
  return <Button type="submit" disabled={pending} className="disabled:cursor-not-allowed disabled:opacity-70">{pending ? "Finalizando..." : "Finalizar nota"}</Button>;
}

function FinalizeClinicalNoteForm({ action, expectedUpdatedAt }: { action: (state: FormState, formData: FormData) => Promise<FormState>; expectedUpdatedAt: string }) {
  const [state, formAction] = useActionState<FormState, FormData>(action, {});
  const cancelRef = useRef<HTMLButtonElement>(null);

  return <form action={formAction} className="p-5 sm:p-6">
    <input type="hidden" name="expected_updated_at" value={expectedUpdatedAt} />
    <input type="hidden" name="expected_status" value="draft" />
    <div className="flex items-start justify-between gap-4">
      <div>
        <h2 id="finalize-clinical-note-title" className="text-lg font-bold text-ink">Finalizar nota clínica</h2>
        <p id="finalize-clinical-note-description" className="mt-2 text-sm leading-6 text-slate-600">Una nota finalizada ya no podrá editarse. Revisa cuidadosamente su contenido antes de continuar.</p>
      </div>
      <button type="button" onClick={() => cancelRef.current?.closest("dialog")?.close()} className="glass-control grid h-9 w-9 shrink-0 place-items-center text-slate-500 hover:text-ink" aria-label="Cerrar diálogo"><X className="h-4 w-4" /></button>
    </div>
    {state.error ? <p role="alert" className="mt-4 rounded-md border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">{state.error}</p> : null}
    <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
      <button ref={cancelRef} autoFocus type="button" onClick={() => cancelRef.current?.closest("dialog")?.close()} className="glass-control inline-flex h-11 items-center justify-center px-4 text-sm font-semibold text-ink">Cancelar</button>
      <FinalizeSubmitButton />
    </div>
  </form>;
}

export function FinalizeClinicalNote({ action, expectedUpdatedAt }: { action: (state: FormState, formData: FormData) => Promise<FormState>; expectedUpdatedAt: string }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  const triggerRef = useRef<HTMLButtonElement>(null);
  const [isOpen, setIsOpen] = useState(false);
  const [attempt, setAttempt] = useState(0);

  function openDialog() {
    setAttempt((value) => value + 1);
    setIsOpen(true);
    dialogRef.current?.showModal();
  }

  return (
    <>
      <button ref={triggerRef} type="button" onClick={openDialog} className="glass-control inline-flex h-11 items-center justify-center gap-2 px-4 text-sm font-semibold text-ink"><CheckCircle2 className="h-4 w-4" />Finalizar nota</button>
      <dialog
        ref={dialogRef}
        role="dialog"
        aria-modal="true"
        aria-labelledby="finalize-clinical-note-title"
        aria-describedby="finalize-clinical-note-description"
        onClose={() => { setIsOpen(false); requestAnimationFrame(() => triggerRef.current?.focus()); }}
        className="glass-card-strong w-[calc(100%-2rem)] max-w-md p-0 shadow-dialog backdrop:bg-slate-950/50 backdrop:backdrop-blur-sm"
      >
        {isOpen ? <FinalizeClinicalNoteForm key={attempt} action={action} expectedUpdatedAt={expectedUpdatedAt} /> : null}
      </dialog>
    </>
  );
}
