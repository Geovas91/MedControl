...
function FinalizeClinicalNoteForm({ action, expectedUpdatedAt }: { action: (state: FormState, formData: FormData) => Promise<FormState>; expectedUpdatedAt: string }) {
  ...
  return <form action={formAction} className="p-5 sm:p-6">
    ...
    <div className="flex items-start justify-between gap-4">
      ...
      <button type="button" onClick={() => cancelRef.current?.closest("dialog")?.close()} className="grid h-9 w-9 shrink-0 place-items-center rounded-md text-slate-500 hover:bg-slate-100 hover:text-ink" aria-label="Cerrar diálogo"><X className="h-4 w-4" /></button>
    </div>
    ...
    <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
      <button ref={cancelRef} autoFocus type="button" onClick={() => cancelRef.current?.closest("dialog")?.close()} className="inline-flex h-11 items-center justify-center rounded-md bg-white px-4 text-sm font-semibold text-ink ring-1 ring-slate-200 hover:bg-slate-50">Cancelar</button>
