...
export function PatientDocumentsTab({ data }: { data: ClinicalRecordData }) {
  ...
  return <section className="glass-card-strong p-4 sm:p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="flex items-center gap-2 text-lg font-bold"><FileSignature className="h-5 w-5 text-clinic" />Documentos clínicos</h2><p className="mt-1 text-sm text-slate-500">CliniControl conserva actualmente consentimientos y sus PDF firmados. No se muestran tipos de documento ficticios.</p></div>{canCreateConsent(data.tenant.membership) ? <ButtonLink href={`/dashboard/patients/${patientId}/consents/new`}><Plus className="h-4 w-4" />Nuevo consentimiento</ButtonLink> : null}</div>
    <div className="mt-5 grid gap-3 md:grid-cols-2">
      {data.consents.length ? data.consents.map((consent) => <article key={consent.id} className="clinical-surface p-4">
