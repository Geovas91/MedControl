...
export const toolSchemas = {
  ...
  searchAppointments: { parse(value: unknown) { const input = record(value); if (!input) return null; const patientId = input.patientId === undefined ? null : uuid(input.patientId); const professionalId = input.professionalId === undefined ? null : uuid(input.professionalId); const date = input.date === undefined ? null : localDate(input.date); const requestedStatus = input.status === undefined ? null : status(input.status); const period: "day" | "upcoming" | null = input.period === undefined ? "day" : input.period === "upcoming" ? "upcoming" : null; return (input.patientId !== undefined && !patientId) || (input.professionalId !== undefined && !professionalId) || (input.date !== undefined && !date) || (input.status !== undefined && !requestedStatus) || !period ? null : { patientId, professionalId, date, status: requestedStatus, period }; } },
