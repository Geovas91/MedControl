...
export const toolSchemas = {
  searchPatients: { parse(value: unknown) { const input = record(value); const query = text(input?.query, 100); return query ? { query } : null; } },
  ...
  availableSlots: { parse(value: unknown) { const input = record(value); const professionalId = uuid(input?.professionalId); const date = localDate(input?.date); const minutes = duration(input?.durationMinutes); return professionalId && date && minutes ? { professionalId, date, durationMinutes: minutes } : null; } },
  createAppointment: { parse(value: unknown) { const input = record(value); const patientId = uuid(input?.patientId); const professionalId = uuid(input?.professionalId); const date = localDate(input?.date); const startTime = localTime(input?.startTime); const minutes = duration(input?.durationMinutes); const title = text(input?.title, 120); return patientId && professionalId && date && startTime && minutes && title ? { patientId, professionalId, date, startTime, durationMinutes: minutes, title } : null; } },
