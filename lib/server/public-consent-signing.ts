...
export async function getPublicConsentByToken(token: string): Promise<PublicConsent | null> {
  ...
  try {
    ...
    const result = await rpc("get_public_consent_for_signing", { token_hash: hashSigningToken(token) });
...
export async function signPublicConsent(token: string, signerName: string, signature: string, acceptedPrivacy: boolean, acceptedSensitiveData: boolean) {
  ...
  const result = await rpc("sign_public_consent", { token_hash: hashSigningToken(token), signer_name: signerName.trim(), signature_png: signature, accepted_privacy: acceptedPrivacy, accepted_sensitive_data: acceptedSensitiveData });
