...
export async function getPatientClinicalBundle(patientId:string): Promise<{state:"ready";data:ClinicalBundle}|{state:"forbidden"|"not_found"|"error";data:null}> {
  ...
  const access=await canAccessClinicalPatientForActiveTenant(patientId); if(access.state!=="ready"||!access.allowed) return {state:access.state==="error"?"error":"forbidden",data:null};
...
export async function saveHistoryForActiveTenant(patientId:string, formData:FormData) {
  ...
  const access=await canAccessClinicalPatientForActiveTenant(patientId); if(access.state!=="ready"||!access.allowed) return {ok:false,error:"No tienes permiso para actualizar la historia clínica."};
...
export async function createVitalForActiveTenant(patientId:string,formData:FormData){
  ...
  const access=await canAccessClinicalPatientForActiveTenant(patientId); if(access.state!=="ready"||!access.allowed) return {ok:false,error:"No tienes permiso para registrar signos vitales."};
  if(!canCreateWithEntitlements(await getClinicEntitlements(context.tenant.clinic.id))) return {ok:false,error:"La clínica no tiene permisos de escritura disponibles."};
