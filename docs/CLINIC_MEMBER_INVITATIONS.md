...
- Desde la migración `0036_identity_payments_integrity.sql`, `auth.users.email` con `email_confirmed_at` es la única fuente de identidad para aceptar. `profiles.email` es descriptivo y editable: no resuelve destinatarios, no autoriza altas y se sincroniza desde la identidad verificada al aceptar.
- `add_clinic_member_by_email_for_current_user` se conserva sólo como símbolo legacy para compatibilidad histórica. No tiene `EXECUTE` para `public`, `anon` ni `authenticated`; la aplicación ya no contiene un helper que lo invoque. Toda alta usa el flujo tokenizado moderno.
