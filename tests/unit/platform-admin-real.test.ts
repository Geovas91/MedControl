...
test("platform admin remains independent from clinic owner and admin roles", () => {
  ...
  assert.match(platformMigration, /where platform_admins\.user_id = auth\.uid\(\)/);
...
test("admin projections are restricted to administrative tables and safe profile fields", () => {
  ...
  assert.doesNotMatch(server, /\.from\("(?:patients|appointments|medical_notes|consents|consent_signatures|payments|audit_logs|bot_logs)"\)/);
...
test("admin v1 is read-only with accessible empty and error states", () => {
  ...
  assert.doesNotMatch(pages, /action=\{|Server Action|createAdminClient/);
});
