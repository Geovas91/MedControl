...
export default async function PublicReviewPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-10">
    <div className="mx-auto max-w-xl">
      ...
      </> : <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm"><h1 className="text-xl font-bold text-ink">Enlace no disponible</h1><p className="mt-3 text-sm text-slate-600">No fue posible validar este enlace. Solicita uno nuevo a la clínica si corresponde.</p></section>}
    </div>
  </main>;
}
