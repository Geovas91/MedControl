...
export default async function ClinicalNotePage({ params, searchParams }: { params: Promise<{ id: string; noteId: string }>; searchParams: Promise<{ note_created?: string | string[]; note_updated?: string | string[]; note_finalized?: string | string[] }> }) {
  ...
  if (result.state !== "ready") return <section className="glass-card-strong p-5 text-sm text-slate-600">No tienes acceso a esta nota clinica.</section>;
    ...
    <>
      ...
      <section className="glass-card-strong p-4 sm:p-6">
        ...
        <dl className="mt-8 grid gap-4 sm:grid-cols-2">
          <div className="clinical-surface p-4"><dt className="text-sm font-semibold text-slate-500">Cita relacionada</dt><dd className="mt-2 text-sm text-ink">{note.appointment_id && note.appointmentTitle ? <Link className="font-semibold text-clinic hover:underline" href={`/dashboard/appointments/${note.appointment_id}`}>{note.appointmentTitle}</Link> : "Sin registro"}</dd></div>
          <div className="clinical-surface p-4"><dt className="text-sm font-semibold text-slate-500">Plantilla</dt><dd className="mt-2 text-sm text-ink">{note.templateName ?? "Sin registro"}</dd></div>
        </dl>
        {note.clinical_impression ? <section className="mt-6"><h2 className="font-bold text-ink">Impresion clinica</h2><p className="clinical-surface mt-3 whitespace-pre-wrap p-4 text-sm leading-6 text-slate-700">{note.clinical_impression}</p></section> : null}
        {note.diagnosis ? <section className="mt-6"><h2 className="font-bold text-ink">Diagnóstico</h2><p className="clinical-surface mt-3 whitespace-pre-wrap p-4 text-sm leading-6 text-slate-700">{note.diagnosis}</p></section> : null}
        {note.icd10_code ? <section className="mt-6"><h2 className="font-bold text-ink">Código ICD-10</h2><p className="clinical-surface mt-3 p-4 text-sm leading-6 text-slate-700">{note.icd10_code}</p></section> : null}
        <section className="mt-6"><h2 className="font-bold text-ink">Contenido</h2><p className="clinical-surface mt-3 whitespace-pre-wrap p-4 text-sm leading-6 text-slate-700">{getClinicalNoteContent(note.note_data)}</p></section>
      </section>
    </>
  );
}
