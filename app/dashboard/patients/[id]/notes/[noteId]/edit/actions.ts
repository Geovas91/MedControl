...
export async function updateClinicalNoteAction(patientId: string, noteId: string, _state: Record<string, unknown>, formData: FormData) {
  ...
  revalidatePath("/dashboard"); revalidatePath("/dashboard/medical-notes"); revalidatePath(`/dashboard/patients/${result.patientId}`); revalidatePath(`/dashboard/patients/${result.patientId}/clinical-record`); revalidatePath(`/dashboard/patients/${result.patientId}/notes/${result.noteId}`);
