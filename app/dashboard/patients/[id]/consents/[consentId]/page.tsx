...
export default async function ConsentDetailPage({ params, searchParams }: { params: Promise<{ id: string; consentId: string }>; searchParams: Promise<Query> }) {
  ...
  if (result.state !== "ready") return <section className="glass-card-strong p-5 text-sm text-slate-600">No tienes acceso a este consentimiento.</section>;
    ...
    <>
      ...
      <section className="glass-card-strong p-4 sm:p-6">
        ...
        {consent.status !== "pending" ? <>
          ...
          <section className="mt-6"><h2 className="font-bold text-ink">Texto firmado</h2><p className="clinical-surface mt-3 whitespace-pre-wrap p-4 text-sm leading-6 text-slate-700">{consent.consent_text}</p></section>
        </> : null}
        ...
        {consent.status === "signed" ? (
          <section className="clinical-surface mt-6 p-4">
