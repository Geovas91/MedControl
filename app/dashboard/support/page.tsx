...
function TicketList({ title, tickets }: { title: string; tickets: NonNullable<Awaited<ReturnType<typeof listMySupportTickets>>["data"]> }) {
  return <section><h2 className="text-lg font-bold text-ink">{title}</h2>{tickets.length ? <div className="mt-3 grid gap-3">{tickets.map((ticket) => <Link key={ticket.id} href={`/dashboard/support/tickets/${ticket.id}`} className="glass-card p-4 transition hover:border-[var(--clinic-border)] hover:bg-white/90 hover:shadow-[var(--shadow-soft)]">
    ...
    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-[var(--foreground-muted)]"><span>{supportCategoryLabels[ticket.category]}</span><Badge variant={ticket.severity === "high" ? "red" : ticket.severity === "normal" ? "amber" : "slate"}>Severidad: {supportSeverityLabels[ticket.severity]}</Badge><Badge variant={ticket.status === "resolved" || ticket.status === "closed" ? "green" : ticket.status === "in_progress" ? "teal" : ticket.status === "waiting_user" ? "amber" : "slate"}>{supportStatusLabels[ticket.status]}</Badge><span>Actualizado {formatSupportDate(ticket.updatedAt)}</span></div>
  </Link>)}</div> : <p className="clinical-surface mt-3 border-dashed p-4 text-sm text-[var(--foreground-muted)]">No hay tickets en esta vista.</p>}</section>;
}
export default async function SupportPage({ searchParams }: SupportPageProps) {
  ...
  return <div className="grid gap-8">
    <section className="glass-card-strong p-6 sm:p-8"><div className="flex items-start gap-4"><div className="glass-control grid h-12 w-12 shrink-0 place-items-center bg-clinic text-white"><LifeBuoy className="h-6 w-6" /></div><div><h1 className="text-2xl font-bold text-ink sm:text-3xl">Ayuda y soporte</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--foreground-soft)]">Soporte técnico para usar CliniControl. No brinda orientación médica. No incluyas nombres de pacientes, diagnósticos ni datos clínicos.</p></div></div></section>

    <section aria-labelledby="buscar-ayuda"><h2 id="buscar-ayuda" className="text-xl font-bold text-ink">Buscar en la base de conocimiento</h2><form className="glass-card mt-3 flex gap-2 p-3"><label className="glass-input flex min-h-11 flex-1 items-center gap-2 rounded-xl px-3"><Search className="h-4 w-4 text-[var(--foreground-muted)]" /><span className="sr-only">¿En qué necesitas ayuda?</span><input name="q" defaultValue={query} maxLength={160} className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="¿En qué necesitas ayuda?" /></label><Button type="submit">Buscar</Button></form>
      {query ? <div className="mt-4">{results.length ? <ul className="grid gap-3">{results.map((article) => <li key={article.slug}><Link href={`/dashboard/support/articles/${article.slug}`} className="glass-card block p-4 transition hover:border-[var(--clinic-border)] hover:bg-white/90"><p className="font-semibold text-ink">{article.title}</p><p className="mt-1 text-sm text-[var(--foreground-muted)]">{getSupportArticleSummary(article)}</p><p className="mt-2 text-xs font-semibold uppercase tracking-wide text-clinic">{article.category}</p></Link></li>)}</ul> : <p role="status" className="clinical-surface border-dashed p-4 text-sm text-[var(--foreground-muted)]">No encontramos resultados publicados para esa búsqueda.</p>}</div> : null}
    </section>

    <section aria-labelledby="temas-frecuentes"><h2 id="temas-frecuentes" className="text-xl font-bold text-ink">Temas frecuentes</h2><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{frequent.map((article) => <Link key={article.slug} href={`/dashboard/support/articles/${article.slug}`} className="glass-card p-4 transition hover:-translate-y-0.5 hover:border-[var(--clinic-border)] hover:shadow-[var(--shadow-soft)]"><BookOpen className="h-5 w-5 text-clinic" /><p className="mt-3 font-semibold text-ink">{article.title}</p><p className="mt-1 text-sm text-[var(--foreground-muted)]">{getSupportArticleSummary(article)}</p></Link>)}</div></section>

    <div className="grid gap-6 lg:grid-cols-2"><section className="glass-card-strong p-5"><h2 className="text-xl font-bold text-ink">Asistente guiado</h2><p className="mb-4 mt-1 text-sm text-[var(--foreground-muted)]">Clasificación determinística; no es un chat ni usa inteligencia artificial.</p><SupportAssistantForm /></section><section className="glass-card-strong p-5"><h2 className="text-xl font-bold text-ink">Diagnóstico seguro</h2><p className="mb-4 mt-1 text-sm text-[var(--foreground-muted)]">Sólo ejecuta comprobaciones permitidas y muestra estados generales.</p><SupportDiagnosticForm /></section></div>

    <section id="crear-ticket" className="glass-card-strong p-5"><h2 className="text-xl font-bold text-ink">Crear ticket</h2><p className="mb-4 mt-1 text-sm text-[var(--foreground-muted)]">La severidad se calcula en el servidor según categoría e impacto.</p><CreateSupportTicketForm /></section>
